package first;

import java.util.Scanner;

public class CustomerManager {

	static final int MAX = 100;		//상수선언, 얼마만큼의 데이터가 들어갈지 모르니 배열의 크기를 100으로 고정
	
	static String[] nameList=new String[MAX];
	static char[] genderList=new char[MAX];
	static String[] emailList=new String[MAX];
	static int[] birthYearList=new int[MAX];
	
	static int count = 0;
	static int index = -1;				//index의 값이 없다='-1'
	
	static Scanner scan = new Scanner(System.in);
	
	
	public static void main(String[] args) {
		while(true) {
			System.out.printf("현재 데이터 갯수 : %d, 현재 커서 위치 %d\n",count,index);
			System.out.println("[I]nsert, [P]revious, [N]ext, [R]ead, [U]pdate, [D]elete, [Q]uit");
			System.out.print("메뉴 입력 : ");
//			String menu = scan.next().toLowerCase().charAt(0);
			String menu = scan.next().toLowerCase();
			switch(menu.charAt(0)) {
			case 'i' : 
			case 'ㅑ' :
				if(count>=MAX) {
					System.out.println("더 이상 입력할 수 없습니다.");
				}else {
					System.out.println("입력을 시작합니다.");
					insertCustomerData();
				}
				break;
			case 'p' : 
			case 'ㅔ' :
				if(index<=0) {
					System.out.println("더 이상 앞으로 이동할 수 없습니다.");
				}else {
					System.out.println("커서를 앞으로 이동합니다.");
					index--;
				}
				break;
			case 'n' : 
			case 'ㅜ' :
				if(index>=count-1) {
					System.out.println("더 이상 뒤로 이동할 수 없습니다.");
				}else {
					System.out.println("커서를 뒤로 이동합니다.");
					index++;
				}
				break;
			case 'r' : 
			case 'ㄱ' :
				if(index >-1 && index < count) {
					System.out.println("데이터를 조회합니다.");
					readCustomerData(index);
				}else {
					System.out.println("커서 위치가 비정상입니다.");
				}
				break;
			case 'u' : 
			case 'ㅕ' :
				if(index >-1 && index < count) {
					System.out.println("데이터를 수정합니다.");
					updateCustomerData(index);
				}else {
					System.out.println("커서 위치가 비정상입니다.");
				}
				break;
			case 'd' : 
			case 'ㅇ' :
				if(index >-1 && index < count) {
					System.out.println("데이터를 삭제합니다.");
					deleteCustomerData(index);
				}else {
					System.out.println("커서 위치가 비정상입니다.");
				}
				break;
			case 'q' : 
			case 'ㅂ' :
				System.out.println("프로그램을 종료합니다.");
				scan.close();       	//Scanner의 메모리 낭비 방지
				System.exit(0);
			default :
				System.out.println("메뉴를 잘못 입력하셨습니다.");
			}
		}

	}
	public static void insertCustomerData() {
		System.out.print("이름 : ");
		String name =scan.next();
		System.out.print("성별(M/F) : ");
		char gender = scan.next().toUpperCase().charAt(0);
		System.out.print("이메일 : ");
		String email = scan.next();
		System.out.println("출생년도(ex:1990) : ");
//		int a = (int)"100"		//참조, 기본은 서로 형변환 불가.
		int birthYear = scan.nextInt();
		nameList[count] = name;
		genderList[count] = gender;
		emailList[count] = email;
		birthYearList[count] = birthYear;
		count++;
		
	}
	
	public static void readCustomerData(int index) {
		System.out.printf("이름 : %s\n", nameList[index]);
		System.out.printf("성별 : %c\n", genderList[index]);
		System.out.printf("이메일 : %s\n", emailList[index]);
		System.out.printf("출생년도 : %d\n", birthYearList[index]);
	}
	
	public static void updateCustomerData(int index) {
		System.out.printf("이름(%s) : ", nameList[index]);
		nameList[index] = scan.next();
		System.out.printf("성별(%c) : ",genderList[index]);
		genderList[index] = scan.next().toUpperCase().charAt(0);
		System.out.printf("이메일(%s) : ", emailList[index]);
		emailList[index] = scan.next();
		System.out.printf("생년월일(%s) : ", birthYearList[index]);
		birthYearList[index] = scan.nextInt();
	}
	
	public static void deleteCustomerData(int index) {
		if(index==count-1) {
			System.out.println("삭제 할 수 없습니다.");
		}else {
			for(int i=index; i<count-1; i++) {
				nameList[i]=nameList[i+1];
				genderList[i]=genderList[i+1];
				emailList[i]=emailList[i+1];
				birthYearList[i]=birthYearList[i+1];
			
			}				
		}
		count--;
	}
}
